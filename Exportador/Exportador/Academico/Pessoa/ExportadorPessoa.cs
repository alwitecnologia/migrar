...
namespace Exportador.Academico.Pessoa
{
    public class ExportadorPessoa : IExportador
    {
        ...
        private static string _queryCountPessoas = @"SELECT COUNT(*)
                                            FROM (" + _queryPessoas + ") AS COUNTER";
        ...
        private bool buscarPessoas(List<Pessoa> pessoas)
        {
            ...
            using (DbCommand command = database.GetSqlStringCommand(_queryPessoas))
            {
                ...
                using (IDataReader drAlunos = database.ExecuteReader(command))
                {
                    processedRecords = 0;
                    ...
                    while (drAlunos.Read())
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            if (_bgWorker != null)
                                _bgWorker.ReportProgress(Convert.ToInt32(processedRecords / totalRecords * 100), String.Format("Não foi possível exportar a pessoa: Código {0},Motivo:{1}.", drAlunos["CODIGO"], ex.Message));
                        }
                        if (_bgWorker != null)
                            _bgWorker.ReportProgress(Convert.ToInt32(processedRecords / totalRecords * 100));
                    }
        ...
        private Pessoa mapearPessoa(IDataRecord record)
        {
            ...
            pessoa.Rua = (DBNull.Value == record["RUA"]) ? String.Empty : ((string)record["RUA"]).Replace(";", String.Empty);
            pessoa.Numero = (DBNull.Value == record["NUMERO"]) ? String.Empty : (string)record["NUMERO"];
            pessoa.Complemento = (DBNull.Value == record["COMPLEMENTO"]) ? String.Empty : ((string)record["COMPLEMENTO"]).Replace(";", String.Empty);
            pessoa.Bairro = (DBNull.Value == record["BAIRRO"]) ? String.Empty : (string)record["BAIRRO"];
            pessoa.Estado = (DBNull.Value == record["ESTADO"]) ? String.Empty : (string)record["ESTADO"];
            pessoa.Cidade = (DBNull.Value == record["CIDADE"]) ? String.Empty : (string)record["CIDADE"];
            pessoa.Pais = (DBNull.Value == record["PAIS"]) ? String.Empty : (string)record["PAIS"];
